...
class receipt_items(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Automatic status update based on missing fields
        if not all([self.sku, self.product_name, self.quantity, self.unit_price, self.total_price, self.raw_text]):
            self.status = 'flagged'
        else:
        ...
        # New item hai to ASINs ke sath match karein
        if is_new:
    ...
    def match_with_asins(self):
        ...
        try:
            ...
            # Matched status update karein
            if matched_any:
                self.matched_status = 'matched'
                # Save without triggering save() again (circular reference avoid)
                receipt_items.objects.filter(id=self.id).update(matched_status='matched')
...
class sticker_data(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not all(required_fields) or any(field is None for field in required_fields):
            self.status = 'flagged'
        else:
            self.status = 'processed'
        super().save(*args, **kwargs)
